...
class Command(BaseCommand):
    help = 'سینک مدل‌های زیربرنامه cantact_app از سرور به لوکال'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("🎯 شروع سینک مدل‌های cantact_app از سرور به لوکال...")
        ...
        # فیلتر کردن فقط مدل‌های cantact_app
        cantact_changes = []
        target_models = []
        ...
        for change in all_changes:
            if change.get('app_name') == 'cantact_app':
                model_type = change.get('model_type')
                if not target_models or model_type in target_models:
                    cantact_changes.append(change)

        self.stdout.write(f"🎯 رکوردهای cantact_app: {len(cantact_changes)}")

        if not cantact_changes:
            self.stdout.write("⚠️ هیچ داده‌ای برای cantact_app یافت نشد")
            return

        # نمایش مدل‌های موجود
        model_counts = {}
        for change in cantact_changes:
        ...
        for i in range(0, len(cantact_changes), batch_size):
            batch = cantact_changes[i:i + batch_size]
            batch_num = (i // batch_size) + 1
            total_batches = (len(cantact_changes) + batch_size - 1) // batch_size
            ...
            try:
                ...
                if errors > 0:
                    ...
                    # نمایش نمونه خطاها
                    for error in result['errors'][:3]:  # فقط ۳ خطای اول
                        self.stdout.write(f"     - {error}")

                # درصد پیشرفت
                progress = min(100, int((i + len(batch)) / len(cantact_changes) * 100))
                self.stdout.write(f"   📈 پیشرفت: {progress}%")

                # تاخیر بین درخواست‌ها
                if i + batch_size < len(cantact_changes):
    ...
    def check_final_status(self):
        ...
        try:
            from cantact_app.models import Branch, BranchAdmin, accuntmodel, savecodphon, dataacont, phonnambermodel

            self.stdout.write(f"\n📋 وضعیت نهایی cantact_app:")

            model_stats = {
                'Branch': Branch.objects.count(),
                'BranchAdmin': BranchAdmin.objects.count(),
                'accuntmodel': accuntmodel.objects.count(),
                'savecodphon': savecodphon.objects.count(),
                'dataacont': dataacont.objects.count(),
                'phonnambermodel': phonnambermodel.objects.count(),
            }
