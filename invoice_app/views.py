...
#         try:
#             data = json.loads(request.body)
#             paid_amount = data.get('paid_amount')  # دریافت مبلغ پرداخت شده
#
...
def finalize_invoice(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # دریافت مبلغ پرداخت شده از درخواست
            paid_amount = total_amount  # مقدار پیش‌فرض
            try:
                request_data = json.loads(request.body)
                paid_amount = request_data.get('paid_amount', total_amount)
                print(f"💰 مبلغ پرداخت شده دریافت شد: {paid_amount}")
            except:
                print("⚠️ استفاده از مبلغ کل به عنوان مبلغ پرداخت شده")
            ...
            invoice = Invoicefrosh.objects.create(
                ...
                is_finalized=True,
                is_paid=True if payment_method in ['cash', 'pos'] else False,
                paid_amount=paid_amount,  # ذخیره مبلغ پرداخت شده
            ...
            print(f"   - مبلغ پرداخت شده: {paid_amount}")
